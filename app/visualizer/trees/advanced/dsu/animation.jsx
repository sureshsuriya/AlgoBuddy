...
export default function DsuAnimation() {
  ...
  const runFindAnimation = (targetId) => {
    ...
    while (true) {
      ...
      // highlight previous path elements
      path.forEach(n => {
        activeHighlights[n] = "active";
      });
      activeHighlights[curr] = "visiting";
      
      // Highlight edge leading up to parent
  ...
  const runUnionAnimation = (x, y) => {
    ...
    if (rootX === rootY) {
      ...
      if (unionByRank) {
        ...
        // Union by rank is off: hook Y to X arbitrarily
        currentParent[rootY] = rootX;
        currentSize[rootX] += currentSize[rootY];
        newSteps.push({
          highlightedNodes: { [rootX]: "matched", [rootY]: "visiting" },
      ...
      // Final step showing updated state
      newSteps.push({
        highlightedNodes: { [rootX]: "matched" },
        highlightedEdges: {},
        parent: [...currentParent],
